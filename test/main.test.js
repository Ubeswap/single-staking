...
contract("VotableStakingRewards", (accounts) => {
  ...
  describe("#constructor/stake", () => {
    it("should work", async () => {
      ...
      const balanceAfterV1 = await token.balanceOf(v1);
      console.log(`v0 balance: ${balanceAfterV0} \n v1 balance: ${balanceAfterV1}`)

      balanceBefore.sub(balanceAfterV0).should.be.eq.BN(amount);
      (await stakingRewards.balanceOf(sender)).should.be.eq.BN(amount);
      (await stakingRewards.balanceOf(v1)).should.be.eq.BN(amount);
      (await stakingRewards.balanceOf(v2)).should.be.eq.BN(amount);
      ...
      (await token.balanceOf(voter1.address)).should.be.eq.BN(amount);
      ...
      (await token.getCurrentVotes(voter1.address)).should.be.eq.BN(amount);
